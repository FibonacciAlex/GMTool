...
public class SecurityRealm extends AuthorizingRealm{
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		User userDataStruct = userService.getUserByName(userName);
		
		//获取用户的角色列表
		List<Role> list = roleService.selectRolesByUser(userDataStruct);
		for (Role role : list) {
			info.addRole(role.getRoleSign());
			List<Permission> pList = pService.getPermissionsByRoleID(role.getID());
			for (Permission p : pList) {
				info.addStringPermission(p.getSign());
			}
		}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		//进行验证
		User dataStruct = userService.getUserByName(userName);
		if(dataStruct == null){
			throw new AuthenticationException("用户不存在！");
		}
		if(!dataStruct.getPwd().equals(pwd)){
